...
var fileMap={};
var webresponse;
//start   deleteIndex   deleteConfig
// 这三步是我自己项目的一个特殊处理，请忽略
// 删除update冲突文件
function start(req,res){
    webresponse=res;
    //删除可能会冲突的文件，index.aspx和config.js
    deleteIndex();
}
function deleteIndex(){
    exec('del '+__dirname+'\\'+config.projectName+'\\Pages\\index.aspx',function(){
        deleteConfig();
    })
}
function deleteConfig(){
    exec('del '+__dirname+'\\'+config.projectName+'\\Scripts\\config.js',function(){
        getVersion();
    })
}
function getVersion(req,res) {
	if(res) webresponse=res;
...
function readFile() {
    ...
    webresponse.write('done'+new Date().getTime());
        ...
        docStr=docStr.replace(/<\s*script\s+[^>]*src=(\"|')([^>]+\.js(?:\?v=\d*)?)\1[^>]*>\s*<\s*\/\s*script\s*>/g,
            function(str,index,initStr){
                ...
                return str.replace(/(?=src\=(\"|')).+\.js(\?v=\d*)?(?=\"|')/g,function(substr){
                    ...
                    return substr.split("?")[0]+"?v="+version;
                });
        });
        //替换用标签加载的css文件
